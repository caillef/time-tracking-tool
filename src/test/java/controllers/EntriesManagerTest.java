...
public class EntriesManagerTest {
    ...
    public void setUp() throws Exception {
        ...
        timeEntry = new TimeEntry(null, task.getId(), new Date(), new Date(), UUID.randomUUID());
    }
    ...
    public void deleteItem() {
        ...
        manager.deleteItem(timeEntry);

        assertEquals(1, manager.getList().size());
        assertEquals(id, manager.getList().get(0).getId());
        manager.deleteItem(new TimeEntry(null, null, null, null, id));
        assertEquals(0, manager.getList().size());
    }
    ...
    public void getGlobalStatistics() {
        ...
        for (int i = 0; i < 10; ++i) {
            manager.addItem(new TimeEntry(null, tasks[0].getId(), date, cal.getTime(), UUID.randomUUID()));
        }
        for (int i = 0; i < 5; ++i) {
            manager.addItem(new TimeEntry(null, tasks[1].getId(), date, cal.getTime(), UUID.randomUUID()));
        }
        cal.add(Calendar.HOUR_OF_DAY, 1); // adds one hour to have two hours time entries
        for (int i = 0; i < 7; ++i) {
            manager.addItem(new TimeEntry(null, tasks[2].getId(), date, cal.getTime(), UUID.randomUUID()));
        }
    ...
    public void getGlobalStatisticsWithRangeHours() {
        ...
        for (int i = 0; i < 2; ++i) {
            manager.addItem(new TimeEntry(null, tasks[1].getId(), cal1.getTime(), cal2.getTime(), UUID.randomUUID()));
        }
        ...
        for (int i = 0; i < 3; ++i) {
            manager.addItem(new TimeEntry(null, tasks[2].getId(), cal3.getTime(), cal4.getTime(), UUID.randomUUID()));
        }

        ArrayList<TimeEntryAggregator> list = manager.getGlobalStatistics(manager.getList().get(5).getStart(), cal2.getTime());
        assertEquals(2*2, list.get(0).getNbHours()/3600000);
        assertEquals(3*2, list.get(1).getNbHours()/3600000);
    }
